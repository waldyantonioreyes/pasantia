...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Validaciones
    if (empty($nombre) || empty($direccion) || empty($codigo) || empty($correo) || empty($telefono) || empty($whatsapp)) {
        $error = "Todos los campos son obligatorios.";
    } elseif (!filter_var($correo, FILTER_VALIDATE_EMAIL)) {
        $error = "El formato del correo no es válido.";
    } else {
        // Verificar si el centro o código ya existen
        $stmt = $conexion->prepare("SELECT id FROM centro_educativo WHERE codigo = ? OR correo = ?");
        $stmt->bind_param("ss", $codigo, $correo);
        $stmt->execute();
        $resultado = $stmt->get_result();

        if ($resultado->num_rows > 0) {
            $error = "El código o correo ya están registrados.";
        } else {
            // Insertar nuevo centro educativo
            $stmt = $conexion->prepare("
                INSERT INTO centro_educativo (nombre, direccion, codigo, correo, telefono, whatsapp)
                VALUES (?, ?, ?, ?, ?, ?)
            ");
            $stmt->bind_param("ssssss", $nombre, $direccion, $codigo, $correo, $telefono, $whatsapp);

            if ($stmt->execute()) {
                $success = "✅ ¡Centro educativo registrado correctamente!";
                header("refresh:2;url=index.php");
            } else {
                $error = "Error al registrar el centro. Intente nuevamente.";
            }
        }
        $stmt->close();
    }
...
?>
<?php
$pageTitle = "Registro de Centros Educativos";
...
<div class="registro-container">
    ...
    <form method="POST" action="">
        <div class="form-group">
            <label><i class="fas fa-school"></i> Nombre del Centro</label>
        ...
        <div class="form-group">
            <label><i class="fas fa-barcode"></i> Código del Centro</label>
